...
class Network(nn.Module):
    ...
    def __init__(self, layer_widths, layer_centres, basis_func):
        ...
        for i in range(len(layer_widths) - 1):
            self.rbf_layers.append(
                ...
                # input : （input_features_num , output_features_num ,function)
            )
            self.linear_layers.append(
                nn.Linear(layer_centres[i], layer_widths[i+1])
            ) # 经过一个linner transform 把上一层输出维度变换到下一层输入维度
    ...
    def fit(self, x, y, epochs, batch_size, lr, loss_func):
        logger = get_logger(
            f'./logs/{time.strftime("%Y_%m_%d_%H_%M_%S_log", time.localtime())}.log'
        )
        ...
        while epoch < epochs:
            ...
            logger.info(
                'Epoch:[{}/{}]\t epoch-loss={:.5f}\t'.format(epoch, epochs, epoch_loss ))
...
x2 = 0.5*np.cos(np.pi*x1) + 0.5*np.cos(4*np.pi*(x1+1)) # <- updown decision boundary
...
for i in range(samples):
    if i < samples//2:
        x[i,1] = np.random.uniform(
            -1, 0.5*np.cos(np.pi*x[i,0]) + 0.5*np.cos(4*np.pi*(x[i,0]+1))
        )
    else:
        x[i,1] = np.random.uniform(
            0.5*np.cos(np.pi*x[i,0]) + 0.5*np.cos(4*np.pi*(x[i,0]+1)), 1
        )
...
# ravel()：拉直数据 , 如果没有必要，不会产生源数据的副本(flatten 会保留原数据,返回数据副本)
...
ty = torch.cat(
    (torch.zeros(samples//2,1), torch.ones(samples//2,1)), dim=0
)
...
rbfnet.fit(tx, ty, 50, samples, 0.01, nn.BCEWithLogitsLoss())
rbfnet.eval()
...
with torch.no_grad():
    preds = (
        torch.sigmoid(
            rbfnet( torch.from_numpy(values).float() )
        )
    ).data.numpy()
